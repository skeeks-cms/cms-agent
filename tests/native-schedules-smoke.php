...
$beforePreview = CmsAgentModel::find()->count();
$check(count($app->cmsAgent->scheduleChanges['create']) === 2, 'Preview counts missing command and native schedules');
$check(CmsAgentModel::find()->count() == $beforePreview, 'Preview does not write records');
$app->cmsAgent->loadAgents()->loadAgents();
$check(array_sum(array_map('count', $app->cmsAgent->scheduleChanges)) === 0, 'No changes after synchronization');
...
$check(count($app->cmsAgent->scheduleChanges['update']) === 1, 'Changed interval counts as one update');
$app->cmsAgent->loadAgents();
$savedSystem = CmsAgentModel::findOne(['name' => 'job:native-daily']);
CmsAgentModel::updateAll(['is_active' => 0, 'is_running' => 1, 'next_exec_at' => 100, 'job_payload' => '{ "site_id" : 123 }'], ['id' => $savedSystem->id]);
$check(array_sum(array_map('count', $app->cmsAgent->scheduleChanges)) === 0, 'Runtime state and equivalent JSON do not count');
$app->cmsAgent->jobs['native-daily']->jobPayload = ['site_id' => '123'];
$check(count($app->cmsAgent->scheduleChanges['update']) === 1, 'Payload scalar type changes count');
$app->cmsAgent->jobs['native-daily']->jobPayload = ['site_id' => 123];
...
$check(count($app->cmsAgent->scheduleChanges['create']) === 2, 'Preview is scoped to the current site');
$app->cmsAgent->loadAgents();
